% === Cours de Java
% === Chapitre : Introduction

\subsection{Concepts}

\imgfullw{../img/coffeemug.jpg}{
	\Large\textbf{\color{white}programmer}\\
	\textit{\color{gray}[proh-gram-er]}\\
	{\large\color{white}an organism that turn caffeine into software}
}{https://www.flickr.com/photos/secret_canadian/22234285/sizes/o/}


\begin{frame}{Définitions}
Définissons les concepts suivants :
\begin{itemize}
\item Un \emph{programme} ?
\item \emph{Programmer} ?
\item Un \emph{langage de programmation} ? 
\item Différence entre \emph{langue} et \textit{langage} ?
\end{itemize}
\end{frame}


\begin{frame}{Un programme}
La seule chose dont est capable un ordinateur est de réaliser extrêmement rapidement des instructions élémentaires
\par\bigskip
Toute tâche qu'on veut lui confier doit donc être préalablement décrite comme une suite séquentielle d'instructions (un programme)
\note[item]{\url{http://en.wikipedia.org/wiki/Source_lines_of_code} donne des exemples de taille de programmes}
\note[item]{On peut faire référence à Code Studio}
\end{frame}


\begin{frame}{Un langage}
\emph{Classes de langages} : machine, assembleur, haut niveau, structuré, 
orienté objet, fonctionnels, orientés aspects\dots

\bigskip
Une classe de langages est adaptée à une classe de problèmes \dots\ 
et ces problèmes évoluent dans le temps \dots

\note[item]{On peut aborder ici l'historique des langages : machine, assembleur, haut niveau, structuré, orienté objet, fonctionnels, orientés aspects\dots}
\end{frame}


\subsection{Traduction}

\begin{frame}{Le problème de la traduction}
Un ordinateur ne comprend que le langage machine. Nécessité d'une \emph{traduction}
\bigskip
\begin{center}
\begin{tabular}{c|c}
\emph{Compilation} & \emph{Interprétation} \\ \hline
\includegraphics[scale=.35]{../img/java-jvm-compil} &
\includegraphics[scale=.35]{../img/java-jvm-interp} \\
\textit{\small traduit d'une traite} & \textit{\small traduit morceau par morceau} \\
\textit{\small avant l'exécution} & \textit{\small au moment de l'exécution} \\
\end{tabular} 
\end{center}
\note[item]{Faire une comparaison des avantages/inconvénients}
\note[item]{Facilité de distribution, rapidité, facilité de développement}
\end{frame}

\full[bluepigment]{
	\begin{center}
		\color{white}
		{\huge\bf Et \sigle{Java} ?}
		\bigskip\\
		{\large Compilé ou interprété ?}
	\end{center}
}


