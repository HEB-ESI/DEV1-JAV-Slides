...
\begin{frame}
	\begin{center}
		Java est \emph{compilé} puis \emph{interprêté}.
	\par
	L'interprêteur Java est la machine virtuelle (\sigle{JVM})
	\par
	Le langage de bas niveau interprêté par la \sigle{JVM} est le \emph{\sigle{bytecode}}
\end{center}
\end{frame}
...
\par
On obtient la version compilée, le \sigle{bytecode} (\code|Hello.class|)
\par
On peut l'exécuter \texttt{\$ java Hello}\\
\texttt{Bonjour !}
\end{frame}
